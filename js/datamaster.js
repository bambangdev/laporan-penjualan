...
export function renderDataMasterHTML() {
    ...
    page.innerHTML = `
        ...
        <div class="max-w-md p-6 bg-white rounded-lg shadow-md mb-8">
            ...
            <form id="dataMasterForm" class="space-y-4">
                <div>
                    ...
                    <select id="masterDataType" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-pink-500 focus:border-pink-500">
                ...
                <div>
                    ...
                    <input type="text" id="masterDataName" required placeholder="Ketik nama baru" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-pink-500 focus:border-pink-500">
                </div>
                <button type="submit" id="saveMasterDataBtn" class="w-full bg-pink-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-pink-700 flex items-center justify-center transition">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div>
                ...
                <div id="hostListContainer" class="bg-white p-4 rounded-lg shadow-md min-h-[100px]"></div>
            </div>
            <div>
                <h3 class="text-lg font-semibold text-gray-800 mb-2">Daftar Admin</h3>
                <div id="adminListContainer" class="bg-white p-4 rounded-lg shadow-md min-h-[100px]"></div>
            </div>
            <div>
                <h3 class="text-lg font-semibold text-gray-800 mb-2">Daftar Staff Treatment</h3>
                <div id="treatmentListContainer" class="bg-white p-4 rounded-lg shadow-md min-h-[100px]"></div>
            </div>
        </div>
    `;
}

function renderMasterList(containerId, list, title) {
    const container = document.getElementById(containerId);
    if (!container) return;
    container.innerHTML = '';
    if (list.length === 0) {
        container.innerHTML = `<p class="text-sm text-gray-500">Tidak ada data ${title}.</p>`;
        return;
    }
    const ul = document.createElement('ul');
    ul.className = 'space-y-2';
    list.sort((a, b) => a.Nama.localeCompare(b.Nama)).forEach(item => {
        const li = document.createElement('li');
        li.className = 'flex justify-between items-center bg-gray-50 p-2 rounded-md';
        li.innerHTML = `
            <span class="text-gray-800">${item.Nama}</span>
            <button data-row-index="${item.rowIndex}" data-name="${item.Nama}" class="delete-master-item text-red-500 hover:text-red-700 text-xs font-medium">Hapus</button>
        `;
        ul.appendChild(li);
    });
...
async function handleDeleteMasterItem(event) {
    ...
    if (!confirm(`Apakah Anda yakin ingin menghapus "${name}"? Data transaksi yang terkait tidak akan terhapus, namun nama ini tidak akan muncul lagi di dropdown.`)) return;
...
export function setupDataMasterPage(masterData) {
    ...
    renderMasterList('treatmentListContainer', treatments, 'Staff Treatment');

    if (dataMasterForm.dataset.listenerAttached) return;
